...
Flight Price Notifier (機票降價通知): users subscribe to a route (Taipei to Tokyo / Seoul / London at first; admins add more from `/admin/routes`) with a target price in TWD, pay NT$300/month through ECPay (綠界) 定期定額, and get an email when the cheapest fare drops to or below the target. Two halves in one repo:
...
4. **Price check.** `pg_cron` job `flight-price-check` (every 30 min) POSTs to `flight-parser` with the service-role key stored in Vault as `flight_service_role_key`. The parser first lazily expires rows (`cancelled` past its period end; `active` more than `RENEWAL_GRACE_DAYS`=7 past it) and emails them, then fetches the cheapest TWD (and supplementary USD) fare per route from Travelpayouts, updates `routes.last_price`, and hands matches for **paying** subscribers only (`active`, or `cancelled` still inside the paid period) to `flight-notification` in batches of 25. Each run writes one row to `flight.parser_runs` (status, duration, API call count, and `issues` such as Travelpayouts 429s or routes with no fare; kept 90 days), which admins see on `/admin/runs` and as a banner on `/admin`; the banner also flags no run for over 65 min (cron broken) and a run stuck in `running` (crashed or timed out). Travelpayouts limits, route-count capacity and how to read the log: `docs/travelpayouts-limits-and-monitoring.md`.
5. **Routes.** `flight.routes` rows are added by admins through `flight-admin-routes` (preview → create): Chinese names are resolved to IATA codes by `resolvePlace()` in `_shared/travelpayouts.ts`, and a route is only saved when the parser's own v3 fare query finds a price (re-run server-side on create, which also takes the Chinese names from that search, never from the client). Codes and names never change after creation (`update` accepts only `is_active`; a wrong route = disable it and add one with other codes), and rows are never deleted; `is_active = false` stops new sign-ups (`flight-subscribe` 409) and hides the route from non-subscribers, while `flight-parser` keeps checking it until no paying subscriber is left.
6. **Alert email.** `flight-notification` dedups against `flight.notification_history` (same price within 24h is skipped; a drop of ≥20% or ≥NT$2,000 re-alerts) and sends via Resend. `flight-status-notification` sends lifecycle emails (`welcome | cancel | expired | payment_failed | renewed`). Both send from `noreply@roberthut.com`. `send-email` is the Supabase Auth "Send Email" hook (auth mails through Resend), unrelated to fare alerts.
...
Admin switch `flight.settings.payment_required` (使用綠界付款, default on; only an explicit `false` turns it off). Off: `flight-subscribe` skips ECPay and writes a free row (`payment_method = 'free'`, `active`, one month, no trade number). Free rows expire at period end with no grace (`flight-parser`), cancel straight to `expired` without calling ECPay, and can re-subscribe without limit; switching payment back on leaves them running until they end. This is the one place besides the ECPay callbacks that writes `active`.
...
- ECPay runs on the shared **stage** merchant (`ECPAY_ENV=stage`; public test HashKey/HashIV). Going live also needs real merchant credentials, `ECPAY_ENV=prod` and a real `SITE_URL` (the fallback landing site and the resubscribe link in emails; moving from the Vercel URL to `https://flights.roberthut.com` on 2026-09-25, and it defaults to `http://localhost:8080` when unset; it is separate from the Supabase Auth "Site URL"). Moving the site to another domain is a checklist, not automatic: see `docs/change-site-url.md`. Edge secrets (`RESEND_API_KEY`, `SEND_EMAIL_HOOK_SECRET`, `TRAVELPAYOUTS_TOKEN`, `ECPAY_*`, optional `SITE_URL`) are set with `supabase secrets set`; do not paste `supabase secrets list` output (use `--output json` and print names only).
...
`auth.users` is shared with unrelated apps (`project-management`, `udemy-coupon`). The trigger `flight.tag_app_metadata_on_signup()` copies `raw_user_meta_data.app` into `raw_app_meta_data.apps`; this app's tag is `APP_NAME = "fare-finder-pro"` and the sign-in page accepts only accounts already tagged and never auto-adds the tag on a correct password. Sign-up and forgot-password take an email only (`src/routes/auth/index.tsx`): a new email gets `signUp` with a throwaway random password and its confirmation link, an existing one (any app) gets a password-reset email, and both land on `/auth/reset`, where the user sets the real password and the tag is added; `/auth/reset` only opens for a session an email link just created (`PASSWORD_RECOVERY` or a recent email `amr`). `README.md` and `docs/shared-supabase-auth.md` describe this same behavior (aligned on 2026-09-25), so change the code and both docs together. The tag is **client-declared, not a permission**: it is a UI route guard only, never key RLS or data access on it (RLS uses `auth.uid() = user_id`). Any new trigger on `auth.users` must skip its side effects when `new.raw_app_meta_data->'apps'` does not include its own app. Full convention: `docs/shared-supabase-auth.md`; open decision B-1 in the handoff doc covers real access control.
...
- Test against `http://localhost:8080`; the production site `https://flights.roberthut.com` (same Vercel project as `fare-finder-pro.vercel.app`) is not the final code, so don't test against or deploy to it unless asked. `/deploy_vercel` is the project's deploy command and confirms preview vs production first.
- Supabase Auth Redirect URLs allow `flights.roberthut.com`, the Vercel domain, Vercel preview wildcard and `http://localhost:8080/**`; sign-up and password reset both redirect to `${origin}/auth/reset?app=${APP_NAME}`. The `?app=` is how the project-wide `send-email` hook picks each app's sender/subject (its `APPS` whitelist, falling back to `user_metadata.app`, then a generic identity); keep it on every auth-mail call.
