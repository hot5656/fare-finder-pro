...
| `no_18_flight_api_limit_and_trace_2026-09-26.txt` | 查 Travelpayouts API 的使用限制，以及陸續增加航線到多少會被限流；每次查價寫一筆 `flight.parser_runs`（狀態、耗時、API 次數、429／查無票價等異常，保留 90 天），admin 在 `/admin/runs` 與總覽橫幅看得到異常；把限制、容量估算與如何閱讀查價紀錄整理成 `docs/travelpayouts-limits-and-monitoring.md`；merge 到 `main` 並 push。 |
| `no_19_v1.1_user_ppt_2026-09-26.txt` | 製作只含免費訂閱的 v1.1 使用者簡報：大綱 `docs/ppt-outline_1.1_user.md`、在 `flights.roberthut.com` 用 `+ppt` 帳號拍截圖 16–32（email 已遮蔽）、產生 `docs/flight-price-notifier_v1.1_user_2026_0926.pptx`（含網址 QR code），產生器整理到 `docs/deck/`；dashboard 新增航線久未更新時的「目前暫無最新票價」提示；提交 v1.0 簡報與截圖 01–15（修正圖片 alt 內含本機路徑）並 push。 |
| `no_20_v1.1_system_ppt_add_fix_test_report_2026-09-26.txt` | 製作全功能 v1.1 system 簡報：大綱 `docs/ppt-outline_1.1_system.md`、26 張 `docs/flight-price-notifier_v1.1_system_2026_0926.pptx`（`docs/deck/build-system.js`，`npm run build:system`）、admin 截圖 33–42，並移除 Lovable 相關內容；測試計畫新增 8.9（admin 後台與查價監控），唯讀補測 P10、SEC-06、去重 log，會寫入的 AD-01／02／07／08／10／11 與 SEC-04 由使用者操作、以 SQL 與 log 驗證，全部通過（114 項：111 通過、3 部分驗證）；修正 D-8（非 admin 直接開 `/admin` 未導回）與 D-9（航線啟用開關反應遲鈍、表格跳動）；航線名稱改為建立後不可修改（`flight-admin-routes` v4）；測試新增的台北 ✈ 大阪已停用。 |
