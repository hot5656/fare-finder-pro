...
const ALT = {
  ...
  "37-admin-routes-add.png": "新增航線：台北到大阪的辨識結果與來回最低價",
...
async function main() {
  ...
  // 14 — Step 3b: paid
  {
    ...
    s.addNotes(
      "付費模式截圖是 2026-09-21 在舊網址拍的（畫面內容與現在相同）。綠界收銀台已裁掉卡片欄位。",
    );
  }
  ...
  // 17 — Admin entry
  {
    ...
    bullets(
      ...
      [
        ...
        "按鈕只是入口；後台資料由資料庫權限把關，一般帳號直接輸入網址也讀不到",
      ],
  ...
  adminSlide(
    "PART 4 ・ 管理員後台",
    "航線管理",
    ["37-admin-routes-add.png", "新增：台北 → 大阪，即時查到 NT$6,992 才能新增"],
    ...
    "截圖時只按了「查詢價格」；大阪航線是之後測試（2026-09-26）時新增的。",
  );
  ...
  adminSlide(
    ...
    "「強制到期」只在測試開關開啟時出現，可把已取消的訂閱立即改為已結束並寄到期信。",
  );
  ...
  // 25 — Test results
  {
    ...
    bullets(
      s,
      [
        "涵蓋前端與登入、抓價比對、寄信去重、綠界付費牆、資料庫與安全；含綠界排程真實續扣",
        "免費模式開關 10 項全數通過（2026-09-24，P10 於 09-26 補測）",
        "2026-09-25 新網域實測註冊與付款，找到並修正 email 含「+」付款後無法啟用的問題",
      ],
      { x: 0.6, y: 4.2, w: 12.1, h: 2.2, fontSize: 15, paraSpaceAfter: 10 },
    );
    s.addNotes(
      "數字取自 docs/test-report.md（2026-09-26 更新；不含免費模式 8.8 與 admin 後台 8.9）。" +
        "admin 後台 12 項：4 通過、3 部分驗證、5 待測。",
    );
  }
