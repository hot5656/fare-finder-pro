...
報告日期：2026-09-21（最後更新 2026-09-26）
...
| 測試期間 | 2026-09-18（M1 驗收）～ 2026-09-21（補測至 91 個用例全部有記錄）；2026-09-24 免費模式；2026-09-25 換網域與註冊流程改版；2026-09-26 唯讀補測與 admin 後台（8.9） |
| 測試環境 | `localhost:8080` ＋ 共用 Supabase 專案 `luugfvsrawnuzwpjvddt` ＋ ECPay **stage**（特店 `3002607`） |
| 測試計畫用例數 | 92（不含 8.8、8.9；原 91 項，2026-09-25 新增 FE-13） |
| ✅ 通過 | **91** |
| 🟡 部分驗證（僅程式碼審查或缺少一環） | **1** |
| ⬜ 未執行（無測試記錄） | **0** |
| ❌ 最終失敗 | **0** |
| 測試中發現並已修正的缺陷 | 9 項（見第 5 章） |
...
2026-09-21 補測後，**沒有任何用例失敗，也沒有用例缺少執行記錄**；2026-09-26 SEC-06、SEC-04 補測通過後，只剩 H3 一項只驗到一部分（🟡，見第 7 章）。
2026-09-24 新增免費模式開關（計畫 8.8）10 項，9 項通過；P10 於 2026-09-26 補測通過，8.8 全數通過。
2026-09-26 新增計畫 8.9（admin 後台與查價監控）12 項：10 項通過、2 項部分驗證（AD-04、AD-06），見 4.3b；過程中找到並修正 D-8（非 admin 直接開 `/admin` 不會被導回）與 D-9（航線啟用開關反應遲鈍），並把航線名稱改為建立後不可修改。上表的 92 項範圍不含 8.8 與 8.9，全部章節見第 3 章。
...
| 2026-09-26 ~05:30Z | **唯讀補測**（Supabase MCP `execute_sql`，交易內 `set local role authenticated`＋`request.jwt.claims`，結尾 `rollback`，不寫入）：P10、SEC-06；`query_logs` 查 dedup log。**admin 後台**：為 v1.1 system 簡報在 `https://flights.roberthut.com` 以 admin 帳號截圖時一併觀察（只按「查詢價格」，未新增航線、未發送、未切換開關）：AD-03、05、06（預覽）、09、12，以及 AD-04 的警告橫幅 | P10、SEC-06 通過；8.9 見 4.3b |
| 2026-09-26 08:18–15:11Z | **會寫入的 admin／安全測試**（正式站，使用者操作，助理以 SQL 與 log 驗證；自動模式擋下助理直接呼叫會寫入的函式，改由使用者在瀏覽器 console 執行測試片段或點畫面）：批次 1 AD-02＋SEC-04（非 admin `k***741`）；批次 2 AD-07 新增 `TPE-OSA`、AD-10 手動發送；批次 3 AD-08 停用與名稱鎖定（`flight-admin-routes` v4 部署後）；批次 4 AD-11 強制到期（事前備份 admin 的 `TPE-LON` 列，測完以 SQL 還原並逐欄比對一致）；另測 AD-01 並修正 D-8、D-9 | 全部通過；見 4.3b、4.5 |
...
| 8.8 免費模式開關（P） | 10 | 10 | 0 | 0 | 0 |
| 8.9 Admin 後台與查價監控（AD） | 12 | 10 | 2 | 0 | 0 |
...
| P10 非 admin 只讀得到 `payment_required` | ✅ | 2026-09-26：以非 admin 使用者 `07e592ba…` 的身分（交易內 `set local role authenticated`＋`request.jwt.claims`，結尾 `rollback`）讀 `flight.settings` → 只有 `["payment_required"]`，`flight.is_admin()` 為 false。對照組：admin `27c894f5…` 讀到 `force_expire_enabled`、`payment_required`、`v1_compare_enabled` 三筆。policy 為 `users can read payment_required`（`key = 'payment_required'`）與 `admins can read settings`（`flight.is_admin()`） |
...
### 4.3b Admin 後台與查價監控（計畫 8.9）— 2026-09-26

第一輪（唯讀）：在 `https://flights.roberthut.com` 以 admin 帳號為 v1.1 system 簡報截圖時觀察（截圖 `docs/ppt-screenshots/33`–`42`），只按「查詢價格」。第二輪（08:18–15:11Z，會寫入）：使用者在正式站操作畫面或在瀏覽器 console 執行測試片段，助理以 SQL 與 function log 驗證，分四批（見第 2 章）。

| ID | 結果 | 證據 |
|---|:-:|---|
| AD-01 非 admin 看不到後台 | ✅ | 非 admin `k***741` 的 dashboard 沒有 Admin 按鈕。**找到缺陷 D-8**：直接開（或重新整理）`/admin` 時停在後台頁（資料受 RLS 限制，只看到自己的 3 筆訂閱、看不到查價紀錄與 admin 設定，但「v1 價格對照」誤顯示為開啟）；從 app 內導向 `/admin` 則會正確導回。修正後（`e6ff247`）localhost 與正式站皆驗證：非 admin 直接開 `/admin` → `/dashboard`；admin 直接開、重新整理、切換分頁皆正常 |
| AD-02 非 admin 呼叫 admin 函式 | ✅ | 08:18Z，`k***741` 的 JWT，各帶一個 admin 才能做的有效 body：`flight-admin-settings`（關強制到期）、`flight-admin-routes`（停用紐約）、`flight-admin-expire`（admin 的倫敦列）、`flight-admin-notify`（admin 的東京列）→ 全部 `403`；設定值與 `updated_at`、紐約啟用狀態、倫敦列、admin 通知紀錄皆未變 |
| AD-03 總覽統計 | ✅ | 截圖 35：有效 2、MRR NT$600、總訂閱 9、付款失敗 0、各狀態 0／2／3／4，與「所有訂閱」頁的 9 列一致（2 筆 `active` 皆為 ECPay） |
| AD-04 查價健康橫幅 | 🟡 | 警告已看到：「最近一次查價有 1 個警告（2026/9/26 下午1:00:01）」，總覽與查價紀錄頁都有。超過 65 分鐘無查價、`running` 超過 10 分鐘兩種錯誤只有程式碼審查（`CRON_STALE_MS`、`RUN_STUCK_MS`，`src/components/admin/shared.ts`） |
| AD-05 查價紀錄頁 | ✅ | 截圖 42：勾選「只看異常」後每列都是 `warning`，issue 為「查無票價 台北 ✈ 倫敦 v3 TWD」（10 月無 TWD 票價）；每輪 4/4 routes、8 次 API、約 3–6 秒。90 天刪除為程式碼審查 |
| AD-06 新增航線：預覽 | 🟡 | 截圖 37：輸入「台北」「大阪」→ 辨識為 `TPE 台北（台灣）`／`OSA 大阪（日本）`（另有 TSA、KIX、ITM 可選），2026-10 來回最低 NT$6,992；未寫入。名稱改為取自辨識結果後（`637a9ed`），localhost 預覽「台北 → 福岡」顯示「將顯示為「台北 ✈ 福岡」」且無名稱輸入框。預覽本身的 422（未知地名、同地）／409（已存在）分支未測；`create` 的 422 `place_mismatch` 見 AD-08 |
| AD-07 新增航線：建立 | ✅ | 09:11Z 在 `/admin/routes` 新增台北 → 大阪：preview 200、create 200，log `route TPE-OSA created by admin … at 6992 TWD`；新列 `tpe-osa`「台北 ✈ 大阪」、啟用、`last_price` 6992。`k***741` 重新整理 dashboard 即出現新卡片並訂閱（目標 20000，歡迎信收到）。**此航線保留**（航線不刪除），測完為停用 |
| AD-08 停用／啟用（名稱與代碼不可改） | ✅ | `flight-admin-routes` v4 部署後：停用大阪 → 仍持有訂閱的 `k***741` 看得到大阪卡片、admin（無訂閱）看不到；以 admin JWT 呼叫：新訂閱停用航線 → `409`；改名 → `400`；改名＋啟用同一請求 → `400`（大阪維持停用、名稱不變）；`create` 帶與地名不符的代碼（台北／福岡＋`OSA`）→ `422 place_mismatch`，未新增。停用期間（11:40–14:35Z）每輪查價仍為 5／5 routes，`k***741` 的大阪比對照常處理（11 次 `skipped (deduped) …/TPE-OSA`）。**D-9**：使用者回報開關反應遲鈍，見第 5 章 |
| AD-09 註冊用戶列表 | ✅ | 截圖 40：8 個帳號，含註冊與驗證時間、最後登入、訂閱數、付費中數；admin 帳號有 Admin 標籤 |
| AD-10 手動發送 | ✅ | 09:13Z 對 `k***741` 的大阪訂閱（免費、目標 20000、現價 6992）按「手動發送」→ `flight-admin-notify` 200 → `flight-notification` 200；`notification_history` 新增 1 筆 TPE-OSA NT$6,992，`triggered_by` 為 admin；使用者收到「台北 → 大阪 降價通知！NT$6,992 已達標」，信末「此通知由客服人員手動觸發」。**限制**：非達標列被伺服器擋下的分支仍為程式碼審查 |
| AD-11 強制到期 | ✅ | 事前備份 admin 的倫敦列（`cancelled`、至 10-19）。14:55:59Z 按「強制到期」→ `expired`、`current_period_end` = 當下，log `force-expired subscription 289e1288… (TPE-LON)`、`expired email sent`（使用者收到「台北 → 倫敦 降價通知已結束」）。同列再呼叫 → `409`（15:05Z，列未變、未再寄信）；關閉開關後再呼叫 → `403`（15:07Z）。之後使用者以 SQL 還原該列，`to_jsonb` 與備份逐欄一致。**開關維持關閉**（依使用者決定） |
| AD-12 暫無最新票價 | ✅ | 截圖 33（正式站，2026-09-26 部署 e552074 後）：台北 ✈ 倫敦 `last_checked_at` 為 2026-09-25 04:00Z（超過 1 天），卡片顯示「目前暫無最新票價，系統仍會持續查詢。」與「上次查到（來回）：NT$20,345」；其他三條航線照常「最後查詢」。原因與 AD-05 的警告相同 |

**強制到期開關**：測試前為開啟（2026-09-24 起）；AD-11 測完於 2026-09-26 15:07Z 關閉，依使用者決定維持關閉。
...
| SEC-04 body 夾帶他人資料 | ✅ | 2026-09-26 08:18Z：以非 admin `k***741` 的 JWT 呼叫 `flight-subscribe`，body 為 `plan_name: tyo-nyc`、`target_price: 1000`，另夾帶 `email: someone-else@example.com`、admin 的 `user_id`、`route: TPE-TYO` → 200；新列屬於 `k***741`、email 為其本人、`route = TYO-NYC`；資料庫沒有該假 email 的列，admin 也沒有 `tyo-nyc` 的列；歡迎信寄到 `k***741` 本人信箱（使用者確認）。測完已取消 |
| SEC-05 使用者直接寫 subscriptions | ✅ | 同 F1 |
| SEC-06 使用者讀 `notification_history` | ✅ | 2026-09-26：同 P10 的做法，以非 admin 使用者 `07e592ba…` 讀 → **0 列**，而該使用者在表中實際有 13 列（owner 查詢確認），也讀不到他人的訂閱（`others_subs_visible = 0`，自己的 3 筆可讀）。對照組：admin 讀到全部 53 列。**更正**：此表現在有一條 policy `admins can read all notification_history`（`flight.is_admin()`，admin 後台通知紀錄用），不是「無 policy」；一般使用者仍讀不到任何列 |
...
---
| D-8 | 非 admin 直接開或重新整理 `/admin` 不會被導回 `/dashboard`，而是看到後台頁面（資料受 RLS 限制，無外洩；但畫面會誤導，例如「v1 價格對照」顯示為開啟）。原因：`/_authenticated/admin` 的守衛在 `cause === "stay"` 時略過 `is_admin` 檢查，而 SSR 後第一次在瀏覽器載入時 `cause` 也是 `"stay"`；`/_authenticated` 的守衛同理會略過 `getUser`／`hasAppAccess` | 2026-09-26 AD-01（`k***741` 直接開 `/admin`） | `e6ff247`：兩個守衛各自記住最後一次通過完整檢查的使用者，`"stay"` 只在是同一個使用者時才略過。localhost 與正式站驗證：非 admin 直接開 `/admin` → `/dashboard`；admin 直接開、重新整理、切換分頁正常 |
| D-9 | `/admin/routes` 的啟用開關要等伺服器回應（1–3 秒）才切換，期間沒有任何提示，使用者以為沒反應而重複點，造成來回切換（log：紐約 off → on → off）；另外表格欄寬隨內容自動計算，狀態或訊息變化時整張表會跳動 | 2026-09-26 AD-08（使用者回報） | 開關改為樂觀更新（點下立即切換、儲存中變淡閃爍、失敗自動還原；儲存中忽略點擊）；表格改固定欄寬（`table-fixed`＋`colgroup`）；錯誤訊息移到表格下方，列高與欄寬都不因儲存或錯誤改變 |
...
| K-7 | 失敗回呼剛好用完排程次數時（G6），同一事件同時寄出「本期扣款失敗，系統會自動再試幾次」與「訂閱已結束」兩封語意矛盾的信（使用者已實際收到） | 使用者困惑；第一封承諾會重試，第二封說已結束。**正式環境幾乎不會發生**：觸發條件是 `TotalSuccessTimes >= ExecTimes`，而結帳設定 `ExecTimes=999`（月繳約 83 年） | **決定（2026-09-21）：暫不修改程式，只在 `flight-ecpay-period` 加註解說明。** 日後若販售有期限的方案，再於此處先算出「已用完次數」，成立時略過 `payment_failed` 信 |
| K-8 | `payment_failed_at` 在寄信**之前**就設定（C3）；Resend 拒絕或暫時失敗時該期的通知永遠不會補寄，`flight-ecpay-period` 仍回 `1|OK` 所以 ECPay 也不重送 | 罕見（Resend 失敗才會發生），但使用者當期不會被通知扣款失敗 | **已處理並驗證通過（2026-09-21）**：`flight-ecpay-period` 已部署為 v4（線上原始碼已讀回確認含 `onFail`，`verify_jwt` 仍為 false，其他函式雜湊未變）。**端對端驗證**（一次性測試帳號＋2 筆 `target_price = 1` 的測試列，自簽失敗回呼，皆回 `1|OK`）：**X（收件地址無效）**：第 1 次回呼 → `renewal failed` → `status notification (payment_failed) rejected: HTTP 502` → 旗標由 `payment_failed_at` 已設定放回 **null**（修正前 C3 會維持已設定）；第 2 次回呼 → 因旗標為空而**再次嘗試寄信**（`flight-status-notification` 又一次 502，log 出現 Resend `422 validation_error`），並再次放開旗標。**Y（收件地址有效）**：第 1 次回呼 → 寄出 1 封（`flight-status-notification` 200，`payment_failed email sent`），旗標**維持已設定**；第 2 次相同回呼 → **沒有任何寄信呼叫**（整個窗口 `flight-status-notification` 共 3 次：502、502、200），信箱只有 1 封「⚠️ 台北 → 首爾 本期扣款失敗」（使用者收件匣）。結論：寄失敗會重試、寄成功仍只寄一次。測試帳號與其兩筆測試列已由使用者執行清理 SQL 刪除並經查詢驗證（`auth.users` 回到 25、訂閱列回到 5 筆）。做法：`_shared/ecpay.ts` 的 `sendStatusEmail` 新增**選用**參數 `onFail`，寄信被拒絕（`flight-status-notification` 回非 2xx，例如 Resend 拒絕時的 502）或呼叫失敗時才執行；`flight-ecpay-period` 的 `payment_failed` 傳入回呼，把 `payment_failed_at` 清回 null。因為 ECPay 對每次失敗重試都會回呼（最多 6 次），下一次回呼就會重新嘗試寄信，「每期只寄一次」在信寄成功時仍然成立（旗標維持設定）。不傳 `onFail` 的呼叫者行為完全不變，所以**只需重新部署 `flight-ecpay-period`**。本機以模擬 `fetch` 對真正的 `_shared/ecpay.ts` 跑 7 個情境全數通過（200、502、401、網路錯誤、未傳回呼×2、回呼本身丟例外）。**代價**：永久無效的收件地址會在每次失敗回呼多試一次（最多約 6 次），無害。**未涵蓋**：`renewed` 信（每期只有一次成功回呼，沒有可重試的來源）與 `welcome`／`cancel` 信，寄失敗仍不會補寄 |
| K-9 | 「用完排程次數 → `expired`」只在**失敗**回呼的路徑上（G6）。若最後一期扣款成功，列維持 `active`（延期一個月），要等 parser 的「逾期 7 天」規則才轉 `expired` | 不影響收費（ECPay 已終止該系列），只是服務多維持一段時間。**正式環境幾乎不會發生**（同 K-7，`ExecTimes=999`） | **決定（2026-09-21）：暫不修改程式，只在 `flight-ecpay-period` 加註解說明。** 日後若販售有期限的方案，再於成功回呼且 `TotalSuccessTimes >= ExecTimes` 時標記為將到期 |
...
**功能上尚未驗證**
1. ~~`flight-ecpay-return` 於真實首扣是否寫入 `total_success_times = 1`~~ **已驗證**：2026-09-25 D-7 修正後在新網域的真實 stage 付款，訂閱轉 `active` 且 `total_success_times = 1`（見第 5 章 D-7）。
2. 真實續扣回呼中金額欄位名稱（`Amount` 或 `amount`；有 fallback 至 `ECPAY_AMOUNT`，顯示金額不受影響）。需要真實續扣：兩筆仍 `active` 的 stage 訂閱約在 2026-10-20、10-22 續扣，屆時一天內查 `flight-ecpay-period` 的 function log（log 只保留約一天）。
3. H3（USD 取價失敗仍以 TWD-only 交付）。（DB-04 已於 2026-09-21 由真實註冊與 `updateUser` 補驗，見 4.4。）
4. 扣款失敗信的完整內文（只看過主旨與摘要；2026-09-21 寄出，可在收件匣打開確認）。降價信（截圖 `29-email-alert.png`）、付費版與免費版取消信（`15`、`32`）的內文已見於簡報截圖；歡迎信與「已結束」兩種、「本期已扣款」的內文先前已確認。ECPay 廠商後台顯示系列已終止（證據僅為 `RtnCode=1`；定期定額查詢頁在 stage 回傳 500／空白，無法作為證據）。
5. ~~「skipped (deduped)」log 行本身沒有親眼看到~~ **已確認（2026-09-26）**：`query_logs` 查 function log，過去 24 小時（09-25 06:00Z～09-26 05:30Z）4 組達標訂閱各出現 48 次 `skipped (deduped) <user_id>/<route>`，即每 30 分鐘一次。

**沒有測試記錄的計畫用例**：無（P10 與 8.9 各項已於 2026-09-26 執行，見 4.3a、4.3b）。2026-09-21 已補測 20 項：FE-01、02、03、05、06、07、08、09、10、11、12、A4、A5、B6、C3、E6、G5、G6、SEC-02、SEC-07（見 4.6、4.7）。
**仍為 🟡 的項目**：H3（USD 取價失敗仍以 TWD-only 交付，僅程式碼審查；要實測需改程式並部署，或把 USD 取價抽到 `_shared/` 後以模擬 `fetch` 在本機測）、8.9 的 AD-04（查價健康橫幅的兩種錯誤狀態只有程式碼審查）、AD-06（預覽的 422／409 分支未測）。SEC-04、SEC-06 已於 2026-09-26 補測通過。
...
**2026-09-26 admin 測試留下的資料**：
- 新航線 `tpe-osa`「台北 ✈ 大阪」（`TPE-OSA`）：航線不刪除，測完為**停用**。`k***741` 名下的大阪訂閱已取消（`expired`），沒有付費者後 parser 不再查它；該使用者 dashboard 仍會看到這張已停用的卡片（「此航線已停止開放訂閱」）。
- `k***741` 的 `tyo-nyc` 與 `tpe-osa` 免費訂閱：15:11Z 取消，皆為 `expired`（取消信已寄出）。
- `notification_history`：1 筆手動發送（TPE-OSA NT$6,992，`triggered_by` = admin），信真的寄出，保留。
- admin 的 `TPE-LON` 列：AD-11 強制到期後已以 SQL 還原，與備份逐欄一致。
- 設定：「測試：強制到期」改為**關閉**；東京 ✈ 紐約維持啟用。
...
- **整體：後端與前端功能都通過，92 個用例中沒有失敗、沒有缺少記錄（2026-09-26 起 90 通過、2 部分驗證）；新增的 admin 後台（8.9）仍有 5 項待測、3 項部分驗證。註冊確認信進垃圾郵件（K-6）已改善（新範本已部署，並在另一個信箱進了收件匣），建議再加 DMARC 並持續觀察；進入 M3 前需完成正式特店與正式前端。（K-3 已於 2026-09-21 處理。）**
...
7. **處理 K-7、K-8、K-9**（生命週期信件的細節）：**K-7、K-9 已處理（2026-09-21）**：因為正式環境 `ExecTimes=999`，幾乎不會觸發，決定不改邏輯，只在 `flight-ecpay-period` 加註解說明（純註解，未重新部署；線上邏輯不變）。**K-8（寄信失敗不重試）已修正並驗證（`onFail` 回呼，`flight-ecpay-period` v4，見 K-8 一列）。**
