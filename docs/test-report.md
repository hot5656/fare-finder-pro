...
2026-09-21 補測後，**沒有任何用例失敗，也沒有用例缺少執行記錄**；2026-09-26 SEC-06 補測通過後，仍有 2 項只驗到一部分（🟡，見第 7 章）。
2026-09-24 新增免費模式開關（計畫 8.8）10 項，9 項通過；P10 於 2026-09-26 補測通過，8.8 全數通過。
2026-09-26 新增計畫 8.9（admin 後台與查價監控）12 項：4 項通過、3 項部分驗證、5 項待測（多數會寫入正式資料，見 4.3b）。上表的 92 項範圍不含 8.8 與 8.9，全部章節見第 3 章。
...
| 8.9 Admin 後台與查價監控（AD） | 12 | 4 | 3 | 5 | 0 |
...
在 `https://flights.roberthut.com` 以 admin 帳號為 v1.1 system 簡報截圖時觀察（截圖 `docs/ppt-screenshots/33`–`42`）。**沒有寫入**：新增航線只按「查詢價格」，沒有按「確認新增」；沒有按手動發送、強制到期或任何開關。

| ID | 結果 | 證據 |
|---|:-:|---|
| AD-01 非 admin 看不到後台 | ⬜ | 只確認 admin 帳號的 dashboard 有 Admin 按鈕（截圖 34）；非 admin 未測 |
| AD-02 非 admin 呼叫 admin 函式 | ⬜ | 未測；四支函式都先查 `flight.admins`，不在其中回 `403 not an admin`（程式碼審查） |
...
| AD-06 新增航線：預覽 | 🟡 | 截圖 37：輸入「台北」「大阪」→ 辨識為 `TPE 台北（台灣）`／`OSA 大阪（日本）`（另有 TSA、KIX、ITM 可選），2026-10 來回最低 NT$6,992；未寫入。422／409 分支未測 |
| AD-07 新增航線：建立 | ⬜ | 未測（會新增一條不能刪除的正式航線） |
| AD-08 停用／啟用（名稱與代碼不可改） | ⬜ | 未測。2026-09-26 起 `flight-admin-routes` 不再接受改名（改為名稱取自辨識結果、建立後不可改），待部署後一併測 |
| AD-09 註冊用戶列表 | ✅ | 截圖 40：8 個帳號，含註冊與驗證時間、最後登入、訂閱數、付費中數；admin 帳號有 Admin 標籤 |
| AD-10 手動發送 | 🟡 | 截圖 39 中只有付費中的列有「手動發送」按鈕；截圖 41 的通知紀錄有 3 筆「手動 Manual」（2026-09-23、09-24、09-25），證明功能曾成功寄出，但沒有對應的測試記錄；非達標列被擋下的分支為程式碼審查 |
| AD-11 強制到期 | ⬜ | 未執行；截圖 36 顯示開關目前為**開啟**，截圖 39 中兩筆 `cancelled` 列有「強制到期」按鈕 |
...
**注意**：「測試：強制到期」開關目前在正式站為**開啟**（最後更新 2026-09-24 22:20）。它只供測試，建議不用時關閉。
...
**沒有測試記錄的計畫用例**：8.9 的 AD-01、AD-02、AD-07、AD-08、AD-11（見 4.3b；P10 已於 2026-09-26 補測通過）。2026-09-21 已補測 20 項：FE-01、02、03、05、06、07、08、09、10、11、12、A4、A5、B6、C3、E6、G5、G6、SEC-02、SEC-07（見 4.6、4.7）。
**仍為 🟡 的項目**：H3（USD 取價失敗仍以 TWD-only 交付，僅程式碼審查；要實測需改程式並部署，或把 USD 取價抽到 `_shared/` 後以模擬 `fetch` 在本機測）、SEC-04（body 夾帶他人 email／user_id，僅驗證了亂送 `route` 被忽略，email／user_id 取自 JWT 為程式碼審查；可用使用者 session 實測，但會建立一筆訂閱並寄信）；8.9 的 AD-04、AD-06、AD-10（見 4.3b）。SEC-06 已於 2026-09-26 補測通過。
