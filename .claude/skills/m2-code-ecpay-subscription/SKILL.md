...
2. `POST` to `https://payment-stage.ecpay.com.tw/Cashier/CreditCardPeriodAction` with `MerchantID`, `MerchantTradeNo`, `Action=Cancel`, `TimeStamp`, and a `CheckMacValue` over them. Success is `RtnCode=1` (`停用成功`). Two codes mean there is **nothing left to stop**, so log them and cancel locally: `90100150 不存在的訂單編號` (a never-paid synthetic order) and **`90100149 該訂單狀態為停用中`** (already deactivated). The second one is easy to miss and it bites in production: ECPay ends a series by itself — every scheduled charge done, 6 consecutive failures, an expired card — and tells you nothing, so the user's later click on 「取消訂閱」 gets 90100149; if you 502 it they are stuck `active` with no way out. **Any other rejection: return 502 and leave the row untouched**, otherwise the user keeps being charged with their alerts switched off. Put the codes in one set (`NOTHING_TO_STOP`) so a newly seen "already stopped" code is a one-line addition, and pass ECPay's `RtnMsg` back as `detail` so the UI can show why.
